...
GO 
CREATE VIEW notesJournalEntriesView AS SELECT n.id, 1 as category, n.name as NoteName, n.start_date, n.finish_date, f.name as FarmName, nt.type, j.id as journalId FROM Notes as n JOIN Fields as f on n.field = f.id JOIN Journals as j on n.journal = j.id JOIN Note_types as nt ON n.type = nt.id;
GO
